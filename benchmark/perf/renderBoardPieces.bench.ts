...
async function main(): Promise<void> {
	const result = await runMemoryBenchmark("renderBoardPieces", () => {
		const [threeContext, display] = createDisplayWithBoardPiecesFixture();
		threeContext.boardPieces = display.pieces;
	});

	writeFileSync("perf/renderBoardPieces.result.json", JSON.stringify(result, null, 2));
	console.log("Benchmark renderBoardPieces saved");
}
