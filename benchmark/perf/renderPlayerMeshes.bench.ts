...
async function main(): Promise<void> {
	const result = await runMemoryBenchmark("renderPlayerMeshes", () => {
		const [threeContext, display] = createDisplayWithBoardPiecesFixture();
		threeContext.playerMeshes = display.players;
	});

	writeFileSync("perf/renderPlayerMeshes.result.json", JSON.stringify(result, null, 2));
	console.log("Benchmark renderPlayerMeshes saved");
}
